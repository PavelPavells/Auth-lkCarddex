...
class Shipment extends Component {
  ...
  render() {
    ...
    //const { shipment, isFetching } = this.props.data;
    //console.log(shipment);
    //if(shipment.data.length === 0 && !isFetching) {
    //  return <Loader />
    //}
    if (!isLoading) {
      return <Loader />;
                  ...
                  <div key={index.pshipment_uuid} className="wrapper-column">
                    ...
                    <div className="toggle-itype-name-shipment">
                      <div className="numbers">
                        {index.create_date.replace(/([^T]+)T([^\.]+).*/g, "$1 $2")} {/* eslint-disable-line */}
                      </div>{" "}
                    </div>
                    <div className="toggle-itype-name-shipment">
